 ...
 * Uses SumoPod AI with dynamic model selection for Japanese-English translation and Romaji conversion
 */
...
/**
 * Translate text using SumoPod AI with selected model
 * @param request - Translation request object
 * @returns Promise<TranslationResponse>
 */
export async function translateWithAI(request: TranslationRequest): Promise<TranslationResponse> {
...
Translate this Japanese text to English:`
        ...
        console.log(`Sending translation request to SumoPod (${selectedModel})...`);
...
export async function testAIConnection(accessToken?: string): Promise<boolean> {
    try {
        const testResult = await translateWithAI({
            text: 'テスト',
            sourceLanguage: 'japanese',
            targetLanguage: 'english',
            accessToken: accessToken
